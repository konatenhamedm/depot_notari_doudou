...
class CourierInterneController extends AbstractController
{
    ...
    /**
     * @Route("/courier_interne/liste", name="courier_interne_liste")
    ...
    public function liste(
        ...
        $totalData = $courierArriveRepository->countAllInterne();
        $totalFilteredData = $courierArriveRepository->countAllInterne($searchValue);
        $data = $courierArriveRepository->getAllInterne($limit, $offset,  $searchValue);
            ...
            $table->add('id', TextColumn::class, [
                'label' => 'Actions', 'field' => 'id', 'orderable' => false, 'globalSearchable' => false, 'className' => 'grid_row_actions', 'render' => function ($value, $context) use ($renders) {
                    ...
                    $options = [
                        ...
                        'actions' => [
                            'edit' => [
                                'url' => $this->generateUrl('courierInterne_edit', ['id' => $value]), 'ajax' => true, 'icon' => '%icon% fe fe-edit', 'attrs' => ['class' => 'btn-success'], 'render' => new ActionRender(function () use ($renders) {
                                    return $renders['edit'];
                                })
                            ],
                            /*  'details' => [
                            ...
                            'delete' => [
                                'url' => $this->generateUrl('courierInterne_delete', ['id' => $value]), 'ajax' => true, 'icon' => '%icon% fe fe-trash-2', 'attrs' => ['class' => 'btn-danger', 'title' => 'Suppression'],  'render' => new ActionRender(function () use ($renders) {
                                    return $renders['delete'];
                                })
                            ],
                            'archive' => [
                                'url' => $this->generateUrl('courierInterne_archive', ['id' => $value]), 'ajax' => true, 'icon' => '%icon% fe fe-file', 'attrs' => ['class' => 'btn-dark', 'title' => 'Archive'], 'render' => new ActionRender(function () use ($renders) {
        ...
        return $this->render('_admin/interne/liste.html.twig', ['datatable' => $table]);
    }
     ...
     * @return Response
     */
    public  function  archive($id, DocumentCourierRepository $repository)
    {
